#Task - Write the else condition to display the message, on the line 101
# "Error","Enter a positive integer" if user enters a negative number.
...
def add_to_cart(food, quantity):
     if quantity.isdigit() and int(quantity) > 0: #using if condition to check if the entered value is number and it is greater than 0
        cart.append({"name": food["name"], "quantity": int(quantity)})
