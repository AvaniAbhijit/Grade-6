#Task - Understand the function add to cart.
...
cart=[] #creating an empty list to add store items inside the cart
...
def add_to_cart(food, quantity):
        cart.append({"name": food["name"], "quantity": int(quantity)}) #add the items inside cart
        print(f"Added {quantity} {food['name']} to cart") #print it on screen
