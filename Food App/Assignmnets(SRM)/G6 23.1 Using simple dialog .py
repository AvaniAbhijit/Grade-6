#Task - Comment line 144 and understand the output.
#Again uncomment the code and run it.
...
def edit_quantity(tree):
  ...
  if selection: #if user has selected any item
    item_id = tree.selection()[0] #pick the id and store it in item_id variable
    item_index = int(tree.item(item_id, 'text')) - 1 #convert the id to index and store it in item_index variable
    quantity_input = simpledialog.askstring("Edit Quantity", f"Enter new quantity for item {item_id}: ", parent=root) #ask user to enter new quantity
