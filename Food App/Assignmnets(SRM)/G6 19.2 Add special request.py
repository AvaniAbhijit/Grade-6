#Task - Write if condition to add topins to the list on the line 98 to 103, depending on the user's input.
#Topins will be extra_cheese, extra_onions, extra_sause.
...
def show_details_popup(food):
    ...
    add_to_cart_button = tk.Button(popup, text="Add to Cart", command=lambda: add_to_cart(food, quantity_entry.get(), extra_cheese_var.get(), no_onions_var.get(), extra_sauce_var.get()))#update the call to send all the data to function
    add_to_cart_button.pack(pady=10)

    close_button = tk.Button(popup, text="Close", command=popup.destroy)
    close_button.pack(pady=10)

def add_to_cart(food, quantity, extra_cheese, no_onions, extra_sauce): #update parameters to input all values
    if quantity.isdigit() and int(quantity) > 0:
        special_requests = [] #create a list to store all the special requests
