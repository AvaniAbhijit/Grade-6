#Task- Understand the code fron 183 to 203.
#Comment the lines and check the output again recomment the code and observe the output.
...
def view_cart():
    ...
    if not cart:
        ...
        edit_button = tk.Button(cart_window, text="Edit Quantity", command=lambda: edit_quantity(tree))
        edit_button.pack(pady=5)


        add_button = tk.Button(cart_window, text="Add Item", command=lambda: add_item(tree))
        add_button.pack(pady=5)


        #adding remove button
        remove_button = tk.Button(cart_window, text="Remove Item", command=lambda: remove_item(tree))
        remove_button.pack(pady=5)

        # Add checkout button
        checkout_button = tk.Button(cart_window, text="Checkout", command=checkout, bg="black", fg="white") #call checkout instead of process_checkout
...
def checkout():
    ...
    email_popup.title("Enter Email Id")

    email_label = tk.Label(email_popup, text="Enter your email id:")
    email_label.pack()
...
def process_checkout():
  ...
  global email_entry
   # Send email
  sender_email = "avani10.dhatrak@gmail.com"
  sender_password = "hzey twbz pgjs qxzp"
  recipient_email = email_entry.get()

  # Construct MIME message
  message = MIMEText(summary) # Use the summary variable here
  message['From'] = sender_email # adding senders mail for From
  message['To'] = recipient_email # adding recipients mail for To
  message['Subject'] = "Yay! We received your order"  #add the subject of the email

  # Connect to SMTP server
  with smtplib.SMTP('smtp.gmail.com', 587) as server: #while it is connected
...
def edit_quantity(tree):
  ...
  if selection:
    ...
    quantity_input = simpledialog.askstring("Edit Quantity", f"Enter new quantity for item {item_id}: ", parent=root)
    if quantity_input is not None and quantity_input.isdigit() and int(quantity_input) > 0:
      quantity = int(quantity_input)
      cart[item_index]['quantity'] = quantity
      tree.item(item_id, values=(cart[item_index]['name'], quantity, cart[item_index]['price'], quantity * cart[item_index]['price']))
    else:
