#Task- Obeserve the edit cart function and understand the work of the code.
...
def edit_quantity(tree):
  ...
  if selection:
    ...
    if quantity_input is not None and quantity_input.isdigit() and int(quantity_input) > 0: #check if the input is a positive integer
      quantity = int(quantity_input) #convert the input to an integer
      cart[item_index]['quantity'] = quantity #update the quantity in the cart
      tree.item(item_id, values=(cart[item_index]['name'], quantity, cart[item_index]['price'], quantity * cart[item_index]['price'])) #update the treeview item
    else:
      messagebox.showerror("Error", "Invalid quantity. Please enter a positive integer.") #show an error message if the input is not a positive integer
