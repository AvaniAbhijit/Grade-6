...
cart = []
cart_window = None
selected_item_index = None  # To track the selected item in the cart
...
def view_cart():
    global cart_window, selected_item_index
    ...
    if not cart:
        ...
        for i, item in enumerate(cart):
            tree.insert('', 'end', text=i+1, values=(item['name'], item['quantity'], f"${item['price']:.2f}", f"${item['quantity'] * item['price']:.2f}"))

        tree.bind("<<TreeviewSelect>>", lambda e: select_item(tree))
...
def select_item(tree):
    ...
    selected_item_index = tree.index(tree.selection()[0])  # Get selected item index
...
def edit_quantity(tree):
    ...
    item = cart[selected_item_index]
    quantity_popup = tk.Toplevel()
    quantity_popup.title("Edit Quantity")

    quantity_label = tk.Label(quantity_popup, text="New Quantity:")
    quantity_label.pack()

    quantity_entry = tk.Entry(quantity_popup)
    quantity_entry.pack()
    quantity_entry.insert(0, str(item['quantity']))  # Pre-fill with current quantity
    ...
    def update_quantity():
        ...
        if new_quantity.isdigit() and int(new_quantity) > 0:
            ...
            quantity_popup.destroy()
            view_cart()  # Refresh cart window to reflect changes
        else:
            messagebox.showinfo("Error", "Enter a positive integer")

    update_button = tk.Button(quantity_popup, text="Update", command=update_quantity)
